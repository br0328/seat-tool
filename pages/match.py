...
page_model = {
    ...
    'column_info': [
        ...
        (f"val{i}", { 'title': f"Match-{i + 1}", 'editable': True, 'dtype': int })
        for i in range(match_col_count)
    ]
}

def init_tab(notebook):
    tab = create_tab(notebook, 'Match', on_tab_selected)
...
def on_tab_selected():
    ...
    person_match_df = load_table('tbl_person_match')
    ...
    for _, person in person_df.iterrows():
        ...
        for _, pm in person_match_df.iterrows():
            if pm['mid'] == mid:
                v = [pm[f"val{i + 1}"] for i in range(match_col_count)]
                break
        
        records.append(tuple(r + v))
    
    df = pd.DataFrame(records, columns = ['surname', 'forename', 'mid'] + [f"val{i}" for i in range(match_col_count)])
...
def on_save_db_clicked():
    ...
    for _, row in df.iterrows():
        ...
        records.append(tuple(
            [mid] +
            [get_int_nzoe(row[f"val{i}"]) for i in range(match_col_count)]
        ))            
    
    person_match_df = pd.DataFrame(records, columns = ['mid'] + [f"val{i + 1}" for i in range(match_col_count)])
    
    if not save_table('tbl_person_match', person_match_df):
    ...
    for i, row in df.iterrows():
        tv.insert(
            '', 'end', values = tuple(
                ...
                [int_nonzero_or_empty(row[f"val{k}"]) for k in range(match_col_count)]
            )
        )
